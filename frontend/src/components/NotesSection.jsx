...
const NotesSection = ({ plantName, userId = "default-user" }) => {
  ...
  // Fetch all notes and categories when component mounts
  useEffect(() => {
    fetchNotes();
    fetchRecentNote();
  ...
  // ✅ FIXED: Safely handle both array and object API responses
  const fetchNotesByCategory = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        // ✅ If backend returns an array (expected)
        data.forEach(item => {
          if (item && item._id && Array.isArray(item.notes)) {
        ...
        // ✅ If backend returns an object (unexpected but handled)
        Object.keys(data).forEach(key => {
          const notesArray = Array.isArray(data[key]) ? data[key] : [];
          categorized[key] = notesArray.filter(note =>
            !plantName || note.plantName === plantName
          );
        });
      } else {
  ...
  const handleCreateNote = async (e) => {
    ...
    try {
      ...
      await axios.post(API_BASE, noteData);
